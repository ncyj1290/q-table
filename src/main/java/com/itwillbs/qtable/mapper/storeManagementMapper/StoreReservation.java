...
public interface StoreReservation {
	...
	int countReservationByStoreIdx(@Param("store_idx") int store_idx);
	
	/* 해당 매장 idx로 예약 목록 가져오는 쿼리문*/
	List<ReservationVO> selectReservationByStoreIdx(@Param("store_idx") int store_idx, @Param("start_row") int start_row, @Param("list_limit") int list_limit);
